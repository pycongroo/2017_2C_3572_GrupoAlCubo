...
texture texturaDecoSangre;

sampler2D s_texturaDecoSangre = sampler_state
{
	Texture = (texturaDecoSangre);
	ADDRESSU = WRAP;
	ADDRESSV = WRAP;
...
texture texturaDecoRoto;

sampler2D s_texturaDecoRoto = sampler_state
{
	Texture = (texturaDecoRoto);
	ADDRESSU = WRAP;
	ADDRESSV = WRAP;
...
texture texturaDecoGarra;

sampler2D s_texturaDecoGarra = sampler_state
{
	Texture = (texturaDecoGarra);
	ADDRESSU = WRAP;
	ADDRESSV = WRAP;
...
float4 ps_DiffuseMap(PS_DIFFUSE_MAP input) : COLOR0
{
	...
	//float4 decoColor = tex2D(s_texturaDecoSangre, input.Texcoord);
	float4 mixedColor = texelColor;
...
//Input del Vertex Shader
struct VS_INPUT_DIFFUSE_MAP_BLOOD
{
	float4 Position : POSITION0;
	float3 Normal : NORMAL0;
	float4 Color : COLOR;
...
//Output del Vertex Shader
struct VS_OUTPUT_DIFFUSE_MAP_BLOOD
{
	float4 Position : POSITION0;
	float2 Texcoord : TEXCOORD0;
	float3 WorldPosition : TEXCOORD1;
...
//Vertex Shader
VS_OUTPUT_DIFFUSE_MAP_BLOOD vs_DiffuseMapBlood(VS_INPUT_DIFFUSE_MAP_BLOOD input)
{
	VS_OUTPUT_DIFFUSE_MAP output;
...
//Pixel Shader
float4 ps_DiffuseMapBlood(PS_DIFFUSE_MAP_BLOOD input) : COLOR0
{
	//Normalizar vectores
	float3 Nn = normalize(input.WorldNormal);
	float3 Ln = normalize(input.LightVec);
...
* Technique DIFFUSE_MAP
*/
technique DIFFUSE_MAP_BLOOD
{
	pass Pass_0
	{
		VertexShader = compile vs_3_0 vs_DiffuseMapBlood();
		PixelShader = compile ps_3_0 ps_DiffuseMapBlood();
	}
}
...
//Input del Vertex Shader
struct VS_INPUT_DIFFUSE_MAP_ROTO
{
	float4 Position : POSITION0;
	float3 Normal : NORMAL0;
	float4 Color : COLOR;
...
//Output del Vertex Shader
struct VS_OUTPUT_DIFFUSE_MAP_ROTO
{
	float4 Position : POSITION0;
	float2 Texcoord : TEXCOORD0;
	float3 WorldPosition : TEXCOORD1;
...
//Vertex Shader
VS_OUTPUT_DIFFUSE_MAP_ROTO vs_DiffuseMapBroken(VS_INPUT_DIFFUSE_MAP_ROTO input)
{
	VS_OUTPUT_DIFFUSE_MAP output;
...
//Pixel Shader
float4 ps_DiffuseMapBroken(PS_DIFFUSE_MAP_ROTO input) : COLOR0
{
	//Normalizar vectores
	float3 Nn = normalize(input.WorldNormal);
	float3 Ln = normalize(input.LightVec);
	...
	//Obtener texel de la textura
	float4 texelColor = tex2D(diffuseMap, input.Texcoord);
	float4 decoColor = tex2D(s_texturaDecoRoto, input.Texcoord);
	float4 mixedColor = texelColor + decoColor*(decoColor.w);
	if ((decoColor.r+decoColor.g+decoColor.b)/3>0.5) {
		mixedColor = texelColor;
	}
...
* Technique DIFFUSE_MAP
*/
technique DIFFUSE_MAP_BROKEN
{
	pass Pass_0
	{
		VertexShader = compile vs_3_0 vs_DiffuseMapBroken();
		PixelShader = compile ps_3_0 ps_DiffuseMapBroken();
	}
}
...
//Input del Vertex Shader
struct VS_INPUT_DIFFUSE_MAP_GARRA
{
	float4 Position : POSITION0;
	float3 Normal : NORMAL0;
	float4 Color : COLOR;
...
//Output del Vertex Shader
struct VS_OUTPUT_DIFFUSE_MAP_GARRA
{
	float4 Position : POSITION0;
	float2 Texcoord : TEXCOORD0;
	float3 WorldPosition : TEXCOORD1;
...
//Vertex Shader
VS_OUTPUT_DIFFUSE_MAP_GARRA vs_DiffuseMapGarra(VS_INPUT_DIFFUSE_MAP_GARRA input)
{
	VS_OUTPUT_DIFFUSE_MAP output;
...
//Pixel Shader
float4 ps_DiffuseMapGarra(PS_DIFFUSE_MAP_GARRA input) : COLOR0
{
	//Normalizar vectores
	float3 Nn = normalize(input.WorldNormal);
	float3 Ln = normalize(input.LightVec);
	...
	float intensity = lightIntensity / distAtten; //Dividimos intensidad sobre distancia (lo hacemos lineal pero tambien podria ser i/d^2)
												  ...
												  //Obtener texel de la textura
	float4 texelColor = tex2D(diffuseMap, input.Texcoord);
	float4 decoColor = tex2D(s_texturaDecoGarra, input.Texcoord);
	float4 mixedColor = texelColor + decoColor*(decoColor.w);
	if ((decoColor.r + decoColor.g + decoColor.b) / 3>0.5) {
		mixedColor = texelColor;
	}
...
* Technique DIFFUSE_MAP
*/
technique DIFFUSE_MAP_GARRA
{
	pass Pass_0
	{
		VertexShader = compile vs_3_0 vs_DiffuseMapGarra();
		PixelShader = compile ps_3_0 ps_DiffuseMapGarra();
	}
}
